...
if ( ! class_exists( 'Tax_Meta_Class' ) ) :
	...
	class Tax_Meta_Class {
		...
		public function __construct( $meta_box ) {
			...
			add_action( 'wp_ajax_set_repeater_ajax', array( $this, 'set_repeater_ajax' ) );
			add_action( 'wp_ajax_nopriv_set_repeater_ajax', 'set_repeater_ajax' );
     ...
     */
        ...
        <img src="';
			...
			echo $this->repeater_block( $field, $meta );
			$this->show_field_end( $field, $meta );
			echo '<img src="';
			if ( $this->_Local_images )
			{
			...
			echo '" alt="' . __( 'Add', 'tax-meta' ) . '" title="' . __( 'Add', 'tax-meta' ) . '" id="add-' . $field['id'] . '" class="add_repeater_field"><br/></div>';

		}

		public function repeater_block( $field, $meta ) {
			ob_start();
			...
			if ( $this->_Local_images )
			{
				echo $this->SelfPath . '/images/remove.png';
			}
			else
			{
				echo 'http://i.imgur.com/g8Duj.png';
			}
			echo '" alt="' . __( 'Remove', 'tax-meta' ) . '" title="' . __( 'Remove', 'tax-meta' ) . '" id="remove-' . $field['id'] . '" class="remove-row"></div>';
		...
		public function set_repeater_ajax() {
			$response = array();
			if ( ! $_POST['items'] )
			{
			...
			if ( ! $_POST['fieldname'] )
			{
				return $response['error'] = 'No fieldname';
			}
			...
			{
				$change_from[] = $fieldname . '[' . $i . ']';
			}

			$html = "";
			$counter = 0;
			foreach($items as $item){
				$replace_html = str_replace( $change_from, $fieldname . '[' . $counter . ']', $item );
				$html         .= '<div class="at-repater-block">';
				$html         .= $replace_html;
				$html         .= '</div>';
		...
		public function show_field_text( $field, $meta ) {
			self::show_field_begin( $field, $meta );
