...
function index_elements(divId){
    var i = 0;
    var items = [];

    $('#' + divId).children($(".at-repater-block")).each(function () {
        if ($(this).html()) {
            console.log($(this));
            items[i] = $(this).html();
            i++;
    ...
    $.ajax({
        ...
        success: function (response) {
            console.log('success');
            if (response.success) {
                $('#' + divId).html(response.success.html);
            } else {
...
jQuery(document).ready(function ($) {
    $repeater = $('.add_repeater_field');
    $remove = $('.remove-row');
    $repeater.on('click', function () {
        var element = $(this).prev().children(':first-child');
        var divId = $(this).prev().attr('id');
        var html = '<div class="at-repater-block">' + element.html() + '</div>';
        $("#"+divId).append(html);

       // index_elements(divId);
        $('#' + divId).sortable({
        ...
        $('body').on('click', $remove, function (e) {
            if ($(e.target).hasClass('remove-row')) {

                $(e.target).closest(".at-repater-block").remove();
            }
        });
